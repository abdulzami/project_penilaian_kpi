    ...
    <div class="card col-xl-12">
        ...
        <div class="card-body">
            <div class="basic-form">
                <form action="{{ route('update-jabatan',$id) }}" method="post">
                    @csrf
                    @method('put')
                        ...
                        <select id="single-select" name="struktural">
                            ...
                            @foreach ($bidangs as $bidang)
                                <option value="{{$hash->encode($bidang->id_bidang)}}"
                                    @if ($hash->encode($jabatans->id_bidang) == $hash->encode($bidang->id_bidang))
                                        selected
                                    @endif
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nama_jabatan" class="form-control" value="{{$jabatans->nama_jabatan}}" placeholder="Masukkan nama jabatan">
                    </div>
                    <div class="form-group">
                        <label>Penilai</label>
                        <select id="single-select2" name="penilai">
                            <option value="">Pilih</option>
                            @foreach ($jabatansall as $jabatan)
                                <option value="{{ $hash->encode($jabatan->id_jabatan) }}"
                                   @if ($hash->encode($jabatan->id_jabatan) == $hash->encode($jabatans->id_penilai))
                                       selected
                                   @endif
