    ...
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Edit KPI Performance - {{ $pegawai->npk }} - {{ $pegawai->nama }}</li>
            </ol>
        </div>
    </div>
@endsection
    ...
    <div class="card col-xl-12">
        ...
        <div class="card-body">
            <div class="basic-form">
                <form action="{{route('bp-update-kpi-performance-sp',$id)}}" method="post">
                    @csrf
                    @method('put')
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                                @foreach ($penilaianperformance as $index => $kpip)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $kpip->kategori }}</td>
                                        ...
                                        <td>
                                            @if ($kpip->satuan == '%')
                                                <input type="number"
                                                    ...
                                                    value="{{$kpip->realisasi}}"
                                                    class="form-control">
                                            @else
                                                <input type="text"
                                                    name="{{ $id }}{{ $hash->encode($kpip->id_performance) }}"
                                                    class="form-control" value="{{$kpip->realisasi}}">
                                            @endif
                                        </td>
                                        <td>{{ $kpip->satuan }}</td>
                                    </tr>
                    ...
                    <center> <button type="submit" class="btn btn-sm btn-primary mt-3">Setujui Banding dan Simpan</button> </center>
                </form>
            </div>
        </div>
    </div>
